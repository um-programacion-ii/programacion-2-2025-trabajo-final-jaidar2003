...
@Serializable
data class ConfirmarVentaRequest(val compradorEmail: String)
...
class VentaRepository(private val client: HttpClient) {
    ...
    suspend fun confirmarVenta(sessionId: String, email: String): ConfirmarVentaResponse {
        val response = client.post("$SERVER_URL/api/ventas/confirmar") {
            header("X-Session-Id", sessionId)
            contentType(ContentType.Application.Json)
            setBody(ConfirmarVentaRequest(email))
        }
        if (response.status.value == 200) {
            return response.body()
        } else {
    ...
    suspend fun getVentas(): List<Venta> {
        val response = client.get("$PROXY_URL/api/endpoints/v1/listar-ventas")
        if (response.status.value == 200) {
            return response.body()
